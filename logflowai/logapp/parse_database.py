...
def filter_logs(query="", level=None, service=None, start_date=None, end_date=None, similarity_threshold=0.1):
    """
    ...
    log_list = list(logs.exclude(id__in=exact_matches.values_list('id', flat=True)))
    if not log_list:
    ...
    log_scores = [
        (log.id, score) 
        for log, score in zip(log_list, similarity_scores) if score >= similarity_threshold
    ]
    ...
    score_cases = [
        When(pk=log_id, then=Value(score))
        for log_id, score in all_scores
    ]
    
    return LogEntry.objects.filter(id__in=[log_id for log_id, _ in all_scores]).annotate(
        similarity=Case(
            *score_cases,
            default=0.0,
...
def get_logs_by_hour(logs, timezone):
    ...
    logs_by_hour = (
        ...
        .annotate(
            hour=ExtractHour(F('timestamp'), tzinfo=timezone)
        )
        .values('hour')
